...
Этот скрипт подключается к CalDAV календарю, получает все встречи на текущий день и создает документ Word с таблицей, содержащей информацию о встречах. При наличии CardDAV адресной книги, email адреса участников автоматически преобразуются в сокращённые имена (Фамилия И.О.).
...
Если дата не указана в качестве параметра командной строки, скрипт запросит её в интерактивном режиме:

```
Введите дату для печати расписания [нажмите Enter для 11.11.25]:
```

Вы можете:
- Нажать **Enter** для использования даты по умолчанию (сегодня)
- Ввести конкретную дату в любом из поддерживаемых форматов (см. ниже)
- Ввести относительное смещение (0, -1, +1 и т.д.)
...
Скрипт создает документ Word с компактной таблицей из 2 колонок:

**Первая колонка (ширина 3 см):**
- Время начала и окончания встречи (через тире)
- На новой строке в скобках - длительность встречи

**Вторая колонка (оставшееся пространство):**
- Первая строка (жирным шрифтом) - название встречи
- Вторая строка - место встречи и ответственные (участники), разделённые запятой
  * Перед списком участников добавляется слово "ответственные"
  * Если у участника есть имя в CardDAV, оно выводится в сокращённом формате: фамилия и инициалы имени и отчества (например, «Иванов И.В.» вместо «Иванов Иван Васильевич»)
  * Если имени нет, показывается email адрес
  * Участники перечисляются через запятую
